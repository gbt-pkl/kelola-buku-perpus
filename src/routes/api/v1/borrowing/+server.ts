...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (type === 'individual' && !student_name)
		return json({ error: 'student_name required for individual' }, { status: 400 });
	if (type !== 'individual' && !class_leader)
		return json({ error: 'class_leader required for class' }, { status: 400 });

	const { data, error } = await supabaseAdmin.from(table).insert(borrowData).select().single();
